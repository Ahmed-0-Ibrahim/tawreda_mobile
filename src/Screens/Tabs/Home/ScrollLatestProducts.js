...
export default ScrollLatestProducts = (props) => {
    ...
    return (
        <View style={{ alignSelf: 'stretch' ,marginTop:scale(15)}}>
            <View  style={{ flexDirection: "row", alignItems: "center",alignContent:"center",justifyContent:"space-between",marginHorizontal:scale(10) }}>
                <Text color={colors.MainBlue} semiBold size={7} >{props.most?t('MostWanted'):t('LatestProducts')}</Text>
               <TouchableOpacity onPress={()=>{
                 Navigation.push({
                    name: props.most?'MostWanted':'NewProducts',
                    options: {
                        statusBar: {
